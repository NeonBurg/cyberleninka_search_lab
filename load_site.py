# coding=utf-8
import requests, json, re, sqlite3, random
...
def init_create_tables():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS Authors (id  INTEGER NOT NULL, FIO TEXT UNIQUE, PRIMARY KEY (id))")
    cursor.execute("CREATE TABLE IF NOT EXISTS Tags (id  INTEGER NOT NULL, name TEXT UNIQUE, PRIMARY KEY (id))")
    cursor.execute("CREATE TABLE IF NOT EXISTS ScienceCategories (id  INTEGER NOT NULL, name TEXT, url TEXT UNIQUE, PRIMARY KEY (id))")
    cursor.execute("CREATE TABLE IF NOT EXISTS ArticlesTags (id  INTEGER NOT NULL, tag_id integer(10) NOT NULL, article_id integer(11) NOT NULL, PRIMARY KEY (id), FOREIGN KEY(article_id) REFERENCES Articles(id), FOREIGN KEY(tag_id) REFERENCES Tags(id))")
    cursor.execute("CREATE TABLE IF NOT EXISTS ArticlesCategories (id  INTEGER NOT NULL, category_id integer(11) NOT NULL, article_id integer(11) NOT NULL, PRIMARY KEY (id), FOREIGN KEY(article_id) REFERENCES Articles(id), FOREIGN KEY(category_id) REFERENCES ScienceCategories(id))")
    cursor.execute("CREATE TABLE IF NOT EXISTS ArticlesAuthors (id  INTEGER NOT NULL, article_id integer(11) NOT NULL, author_id integer(11) NOT NULL, PRIMARY KEY (id), FOREIGN KEY(article_id) REFERENCES Articles(id), FOREIGN KEY(author_id) REFERENCES Authors(id))")
...
def insert_author(author_FIO):
    cursor.execute("SELECT id FROM Authors WHERE FIO = ?", [author_FIO])
    result = cursor.fetchall()
    author_id = 0
    if len(result) > 0:
        author_id = result[0][0]
    else:
        cursor.execute("INSERT INTO Authors(FIO) VALUES(?)", [author_FIO])
        cursor.execute("SELECT id FROM Authors WHERE FIO = ?", [author_FIO])
...
def save_article(parsed_article):
    ...
    if len(article_result) > 0:
        ...
        cursor.execute("INSERT INTO Articles(title, url, gost_link, journal_id) VALUES(?, ?, ?, ?)", [parsed_article['title'], parsed_article['url'], str(parsed_article['gost_link']), journal_id])
        cursor.execute("SELECT id FROM Articles WHERE url = ?", [parsed_article['url']])
    ...
    for author_fio in authors_list:
        author_id = insert_author(author_fio)
